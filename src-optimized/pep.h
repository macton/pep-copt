...
#define _PEP_FREQ_BLOCK_BITS 4
#define _PEP_FREQ_BLOCK_N ( 1u << _PEP_FREQ_BLOCK_BITS )
#define _PEP_FREQ_BLOCK_MASK ( _PEP_FREQ_BLOCK_N - 1u )
#define _PEP_FREQ_BLOCK_COUNT ( ( PEP_FREQ_N + _PEP_FREQ_BLOCK_N - 1u ) >> _PEP_FREQ_BLOCK_BITS )
...
#define _PEP_ENCODE_MODEL_ABANDONED UINT64_MAX
#define _PEP_MODEL_RAW_INDEX 3u
...
typedef struct
{
	...
	uint16_t block_sum[ _PEP_FREQ_BLOCK_COUNT ];
	uint32_t sum;
}
_pep_context;
...
static void _pep_context_add_freq( _pep_context* const context, const uint32_t symbol, const uint16_t delta );
static void _pep_context_rebuild_blocks( _pep_context* const context );
static _pep_prob _pep_get_prob_from_context( const _pep_context* const context, const uint32_t symbol );
static inline _pep_prob _pep_get_prob_for_escape_from_context( const _pep_context* context, uint32_t escape_symbol );
static void _pep_arith_encode( _pep_ac_encode* const ac, const _pep_prob prob );
static uint8_t _pep_arith_encode_emit_byte( _pep_ac_encode* const ac, const uint8_t byte );
static uint8_t _pep_arith_encode_normalize( _pep_ac_encode* const ac );
static uint8_t _pep_arith_encode_finish( _pep_ac_encode* const ac );
...
static void _pep_context_add_freq( _pep_context* const context, const uint32_t symbol, const uint16_t delta )
{
	context->freq[ symbol ] += delta;
	context->sum += delta;
	context->block_sum[ symbol >> _PEP_FREQ_BLOCK_BITS ] += delta;
}

static void _pep_context_rebuild_blocks( _pep_context* const context )
{
	for( uint32_t block = 0; block < _PEP_FREQ_BLOCK_COUNT; ++block )
	{
		context->block_sum[ block ] = 0;
	}

	for( uint32_t symbol = 0; symbol < PEP_FREQ_N; ++symbol )
	{
		context->block_sum[ symbol >> _PEP_FREQ_BLOCK_BITS ] += context->freq[ symbol ];
	}
}
...
static _pep_prob _pep_get_prob_from_context( const _pep_context* const context, const uint32_t symbol )
{
	...
	const uint32_t block = symbol >> _PEP_FREQ_BLOCK_BITS;
	for( uint32_t index = 0; index < block; ++index )
	{
		prob.low += context->block_sum[ index ];
	}

	const uint32_t block_start = block << _PEP_FREQ_BLOCK_BITS;
	for( uint32_t index = block_start; index < symbol; ++index )
	{
...
static inline _pep_prob _pep_get_prob_for_escape_from_context( const _pep_context* context, uint32_t escape_symbol )
{
	_pep_prob prob;
	prob.scale = context->sum;
	prob.high = context->sum;
	prob.low = context->sum - context->freq[ escape_symbol ];
	return prob;
}
...
static uint8_t _pep_arith_encode_emit_byte( _pep_ac_encode* const ac, const uint8_t byte )
{
	...
	if( ac->abandon_at != 0 && ac->bytes_size >= ac->abandon_at )
	{
		ac->abandoned = 1;
		return 0;
	}
...
static uint8_t _pep_arith_encode_normalize( _pep_ac_encode* const ac )
{
	while( 1 )
	{
		...
		if( !_pep_arith_encode_emit_byte( ac, byte ) ) return 0;
	}
...
#define _PEP_UPDATE( CONTEXT, SYMBOL, FREQ_MAX, PALETTE_SIZE )\
	do\
	{\
		_pep_context_add_freq( CONTEXT, SYMBOL, 2 );\
		...
		{\
			...
			_pep_context_rebuild_blocks( CONTEXT );\
		}\
	}\
	while( 0 )
...
typedef struct
{
	_pep_decode_context* contexts;
	uint32_t index_end;
	uint32_t index_escape;
	int8_t kind;
}
_pep_decode_model_context;
...
static uint8_t _pep_model_init( _pep_model_context* m, int8_t kind, uint32_t pcount )
{
	...
	order0->sum = pcount;
	_pep_context_rebuild_blocks( order0 );
	return 1;
}
...
static _pep_context* _pep_build_level_hash_padded( const _pep_model_context* m, const uint16_t* p, uint32_t stride )
{
	const uint32_t Wv = *( p - 1u );
	const uint32_t Nv = *( p - stride );
	const uint32_t NWv = *( p - stride - 1u );
	const uint32_t NEv = *( p - stride + 1u );
	const uint32_t NNv = *( p - stride * 2u );
	const uint32_t WWv = *( p - 2u );
	const uint32_t NNWv = *( p - stride * 2u - 1u );
	const uint32_t NNEv = *( p - stride * 2u + 1u );
	const uint32_t NWWv = *( p - stride - 2u );
...
static _pep_context* _pep_build_level_nw_padded( const _pep_model_context* m, const uint16_t* p, uint32_t stride )
{
...
static _pep_context* _pep_build_level_w_padded( const _pep_model_context* m, const uint16_t* p, uint32_t stride )
{
	const uint32_t Wv = *( p - 1u );
...
static void _pep_decode_context_add_freq( _pep_decode_context* const context, const uint32_t symbol, const uint16_t delta )
{
	context->freq[ symbol ] += delta;
	context->sum += delta;
}

static void _pep_decode_context_update( _pep_decode_context* const context, const uint32_t symbol, uint16_t* const freq_max, const uint32_t palette_size )
{
	_pep_decode_context_add_freq( context, symbol, 2 );
	if( context->freq[ symbol ] >= *freq_max || context->sum >= PEP_PROB_MAX_VALUE )
	{
		*freq_max += ( PEP_FREQ_END - palette_size ) >> 1;
		context->sum = 0;
...
static _pep_sym_decode _pep_decode_get_sym_from_freq( const _pep_decode_context* const context, const uint32_t target_freq )
{
...
static uint8_t _pep_decode_model_init( _pep_decode_model_context* m, int8_t kind, uint32_t pcount )
{
	memset( m, 0, sizeof( *m ) );
	m->kind = kind;
	m->index_escape = pcount;
	...
	m->contexts = ( _pep_decode_context* )calloc( off, sizeof( _pep_decode_context ) );
	if( !m->contexts ) return 0;

	_pep_decode_context* order0 = &m->contexts[ 0 ];
	for( uint32_t index = 0; index < pcount; index++ ) order0->freq[ index ] = 1;
	order0->sum = pcount;
	return 1;
}

static void _pep_decode_model_free( _pep_decode_model_context* m )
{
	if( m->contexts )
	{
		free( m->contexts );
...
static _pep_decode_context* _pep_decode_build_level_hash( const _pep_decode_model_context* m, const uint8_t* idx, uint32_t pos, uint32_t x, uint32_t y, uint16_t width )
{
...
static _pep_decode_context* _pep_decode_build_level_nw( const _pep_decode_model_context* m, const uint8_t* idx, uint32_t pos, uint32_t x, uint32_t y, uint16_t width )
{
	const uint32_t SENT = m->index_escape;
	const uint32_t index_end = m->index_end;
	const uint32_t Wv = ( x > 0 ) ? idx[ pos - 1 ] : SENT;
...
static _pep_decode_context* _pep_decode_build_level_w( const _pep_decode_model_context* m, const uint8_t* idx, uint32_t pos, uint32_t x, uint32_t y, uint16_t width )
{
	const uint32_t SENT = m->index_escape;
	const uint32_t Wv = ( x > 0 ) ? idx[ pos - 1 ] : SENT;
...
static _pep_decode_context* _pep_decode_build_level( const _pep_decode_model_context* m, const uint8_t* idx, uint32_t pos, uint32_t x, uint32_t y, uint16_t width )
{
	switch( m->kind )
	{
		case _PEP_TAP_HASH:
		{
			return _pep_decode_build_level_hash( m, idx, pos, x, y, width );
		}
		case _PEP_TAP_NW:
		{
			return _pep_decode_build_level_nw( m, idx, pos, x, y, width );
		}
		case _PEP_TAP_W:
		{
			return _pep_decode_build_level_w( m, idx, pos, x, y, width );
		}
	}
	return NULL;
}
...
static uint64_t _pep_encode_model( const uint8_t* idx, const uint16_t* idx_pad, uint32_t idx_pad_stride, uint16_t width, uint16_t height, uint32_t pcount, uint8_t model, uint8_t* out, uint64_t abandon_at )
{
	_pep_model_context m;
	if( !_pep_model_init( &m, _pep_models[ model ], pcount ) ) return _PEP_ENCODE_MODEL_ABANDONED;
	const uint32_t index_escape = m.index_escape;

	_pep_ac_encode ac = { 0 };
	ac.range = ( uint32_t )( ( 1llu << 32 ) - 1 );
	ac.data_ref = out;
	ac.abandon_at = abandon_at;
	uint16_t freq_max = PEP_FREQ_MAX;
	_pep_context* order0 = &m.contexts[ 0 ];
	uint64_t size = _PEP_ENCODE_MODEL_ABANDONED;

#define _PEP_ENCODE_MODEL_LOOP( BUILD_LEVEL )\
	do\
	{\
		uint32_t pos = 0;\
		...
		{\
			...
			{\
				...
				const uint16_t* pad = idx_pad + ( y + 2u ) * idx_pad_stride + ( x + 2u );\
				_pep_context* context = BUILD_LEVEL( &m, pad, idx_pad_stride );\
...
\
				...
				{\
					...
					{\
						...
						if( !_pep_arith_encode_normalize( &ac ) ) goto cleanup;\
						_PEP_UPDATE( context, symbol, freq_max, pcount );\
						encoded_in_level = 1;\
					}\
					else\
					{\
						_pep_prob prob = _pep_get_prob_for_escape_from_context( context, index_escape );\
						_pep_arith_encode( &ac, prob );\
						if( !_pep_arith_encode_normalize( &ac ) ) goto cleanup;\
						_pep_context_add_freq( context, index_escape, 1 );\
					}\
				}\
\
				if( !encoded_in_level )\
...
\
					...
					{\
						...
						context->sum += 1;\
						context->block_sum[ index_escape >> _PEP_FREQ_BLOCK_BITS ] = 1;\
						context->block_sum[ symbol >> _PEP_FREQ_BLOCK_BITS ] += 1;\
					}\
					else\
					{\
						_pep_context_add_freq( context, symbol, 1 );\
					}\
				}\
			}\
		}\
	...
	switch( _pep_models[ model ] )
	{
		case _PEP_TAP_HASH:
		{
			_PEP_ENCODE_MODEL_LOOP( _pep_build_level_hash_padded );
			break;
		}
		case _PEP_TAP_NW:
		{
			_PEP_ENCODE_MODEL_LOOP( _pep_build_level_nw_padded );
			break;
		}
		case _PEP_TAP_W:
		{
			_PEP_ENCODE_MODEL_LOOP( _pep_build_level_w_padded );
			break;
		}
	}

#undef _PEP_ENCODE_MODEL_LOOP

	if( !_pep_arith_encode_finish( &ac ) ) goto cleanup;
	size = ac.bytes_size;
...
// Encode W/NW models directly from the byte index image. This is exact for
// those two models because their taps only need W and N, whose boundary
// sentinels match the padded representation. Hash still uses the padded path.
static uint64_t _pep_encode_model_direct_wnw( const uint8_t* idx, uint16_t width, uint16_t height, uint32_t pcount, uint8_t model, uint8_t* out )
{
	_pep_model_context m;
	if( !_pep_model_init( &m, _pep_models[ model ], pcount ) ) return _PEP_ENCODE_MODEL_ABANDONED;
	const uint32_t index_escape = m.index_escape;

	uint8_t* ac_data_ref = out;
	uint32_t ac_low = 0;
	uint32_t ac_range = ( uint32_t )( ( 1llu << 32 ) - 1 );
	uint16_t freq_max = PEP_FREQ_MAX;
	_pep_context* order0 = &m.contexts[ 0 ];
	uint64_t size = _PEP_ENCODE_MODEL_ABANDONED;
	const int8_t kind = _pep_models[ model ];
	const uint32_t index_end = m.index_end;

#define _PEP_DIRECT_ARITH_ENCODE( PROB ) \
	do \
	{ \
		const _pep_prob direct_prob = ( PROB ); \
		ac_range /= direct_prob.scale; \
		ac_low += direct_prob.low * ac_range; \
		ac_range *= direct_prob.high - direct_prob.low; \
		while( 1 ) \
		{ \
			if( ( ac_low ^ ( ac_low + ac_range ) ) >= PEP_CODE_MAX_VALUE ) \
			{ \
				if( ac_range >= PEP_PROB_MAX_VALUE ) break; \
				ac_range = PEP_PROB_MAX_VALUE - ( ac_low & ( PEP_PROB_MAX_VALUE - 1 ) ); \
			} \
			*ac_data_ref++ = ( uint8_t )( ac_low >> PEP_CODE_BITS ); \
			ac_low <<= PEP_CODE_BITS_INV; \
			ac_range <<= PEP_CODE_BITS_INV; \
		} \
	} \
	while( 0 )

#define _PEP_DIRECT_UPDATE_256( CONTEXT, SYMBOL ) \
	do \
	{ \
		_pep_context* direct_update_context = ( CONTEXT ); \
		const uint32_t direct_update_symbol = ( SYMBOL ); \
		direct_update_context->freq[ direct_update_symbol ] += 2; \
		direct_update_context->sum += 2; \
		direct_update_context->block_sum[ direct_update_symbol >> _PEP_FREQ_BLOCK_BITS ] += 2; \
		if( direct_update_context->freq[ direct_update_symbol ] >= freq_max || direct_update_context->sum >= PEP_PROB_MAX_VALUE ) \
		{ \
			direct_update_context->sum = 0; \
			for( uint64_t f = 0; f < PEP_FREQ_N; f++ ) \
			{ \
				const uint16_t _f = direct_update_context->freq[ f ]; \
				if( _f == 0 ) continue; \
				const uint16_t scaled = ( _f + 1 ) >> 1; \
				direct_update_context->freq[ f ] = scaled; \
				direct_update_context->sum += scaled; \
			} \
			_pep_context_rebuild_blocks( direct_update_context ); \
		} \
	} \
	while( 0 )

#define _PEP_DIRECT_ENCODE_SYMBOL_256( CONTEXT_PTR, SYMBOL_VALUE ) \
	do \
	{ \
		_pep_context* ctxp = ( CONTEXT_PTR ); \
		const uint32_t direct_symbol = ( SYMBOL_VALUE ); \
		const uint16_t direct_freq = ctxp->freq[ direct_symbol ]; \
		if( direct_freq != 0 ) \
		{ \
			_PEP_DIRECT_ARITH_ENCODE( _pep_get_prob_from_context( ctxp, direct_symbol ) ); \
			_PEP_DIRECT_UPDATE_256( ctxp, direct_symbol ); \
		} \
		else \
		{ \
			const uint32_t direct_old_sum = ctxp->sum; \
			if( direct_old_sum != 0 ) \
			{ \
				_PEP_DIRECT_ARITH_ENCODE( _pep_get_prob_for_escape_from_context( ctxp, index_escape ) ); \
				_pep_context_add_freq( ctxp, index_escape, 1 ); \
			} \
			_PEP_DIRECT_ARITH_ENCODE( _pep_get_prob_from_context( order0, direct_symbol ) ); \
			_PEP_DIRECT_UPDATE_256( order0, direct_symbol ); \
			if( direct_old_sum == 0 ) \
			{ \
				ctxp->freq[ index_escape ] = 1; \
				ctxp->sum = 1; \
				ctxp->freq[ direct_symbol ] = 1; \
				ctxp->sum += 1; \
				ctxp->block_sum[ index_escape >> _PEP_FREQ_BLOCK_BITS ] = 1; \
				ctxp->block_sum[ direct_symbol >> _PEP_FREQ_BLOCK_BITS ] += 1; \
			} \
			else \
			{ \
				_pep_context_add_freq( ctxp, direct_symbol, 1 ); \
			} \
		} \
	} \
	while( 0 )

	if( pcount == PEP_FREQ_END )
	{
		if( kind == _PEP_TAP_W )
		{
			uint32_t pos = 0;
			for( uint32_t y = 0; y < height; y++ )
			{
				_pep_context* first_context = &m.contexts[ 1u + index_escape ];
				_PEP_DIRECT_ENCODE_SYMBOL_256( first_context, idx[ pos ] );
				pos++;

				for( uint32_t x = 1; x < width; x++, pos++ )
				{
					_pep_context* context = &m.contexts[ 1u + idx[ pos - 1u ] ];
					_PEP_DIRECT_ENCODE_SYMBOL_256( context, idx[ pos ] );
				}
			}
		}
		else
		{
			_pep_context* nw_rows[ PEP_FREQ_N ];
			for( uint32_t n = 0; n < index_end; n++ )
			{
				nw_rows[ n ] = &m.contexts[ 1u + n * index_end ];
			}

			uint32_t pos = 0;

			_pep_context* first_context = &nw_rows[ index_escape ][ index_escape ];
			_PEP_DIRECT_ENCODE_SYMBOL_256( first_context, idx[ pos ] );
			pos++;

			for( uint32_t x = 1; x < width; x++, pos++ )
			{
				_pep_context* context = &nw_rows[ index_escape ][ idx[ pos - 1u ] ];
				_PEP_DIRECT_ENCODE_SYMBOL_256( context, idx[ pos ] );
			}

			for( uint32_t y = 1; y < height; y++ )
			{
				_pep_context* row_first_context = &nw_rows[ idx[ pos - width ] ][ index_escape ];
				_PEP_DIRECT_ENCODE_SYMBOL_256( row_first_context, idx[ pos ] );
				pos++;

				for( uint32_t x = 1; x < width; x++, pos++ )
				{
					_pep_context* context = &nw_rows[ idx[ pos - width ] ][ idx[ pos - 1u ] ];
					_PEP_DIRECT_ENCODE_SYMBOL_256( context, idx[ pos ] );
				}
			}
		}
		...
		size = ( uint64_t )( ac_data_ref - out );

#undef _PEP_DIRECT_ENCODE_SYMBOL_256
#undef _PEP_DIRECT_UPDATE_256
...
#define _PEP_DIRECT_ENCODE_SYMBOL( CONTEXT_PTR, SYMBOL_VALUE ) \
	do \
	{ \
		_pep_context* ctxp = ( CONTEXT_PTR ); \
		const uint32_t direct_symbol = ( SYMBOL_VALUE ); \
		...
		if( ctxp->sum != 0 ) \
		{ \
			if( ctxp->freq[ direct_symbol ] != 0 ) \
			{ \
				_PEP_DIRECT_ARITH_ENCODE( _pep_get_prob_from_context( ctxp, direct_symbol ) ); \
				_PEP_UPDATE( ctxp, direct_symbol, freq_max, pcount ); \
				encoded_in_level = 1; \
			} \
			else \
			{ \
		...
		{ \
			_PEP_DIRECT_ARITH_ENCODE( _pep_get_prob_from_context( order0, direct_symbol ) ); \
			_PEP_UPDATE( order0, direct_symbol, freq_max, pcount ); \
			if( ctxp->sum == 0 ) \
			{ \
				ctxp->freq[ index_escape ] = 1; \
				ctxp->sum = 1; \
	...
	if( kind == _PEP_TAP_W )
	{
		...
		{
			_pep_context* first_context = &m.contexts[ 1u + index_escape ];
			_PEP_DIRECT_ENCODE_SYMBOL( first_context, idx[ pos ] );
			pos++;

			for( uint32_t x = 1; x < width; x++, pos++ )
			{
				_pep_context* context = &m.contexts[ 1u + idx[ pos - 1u ] ];
				_PEP_DIRECT_ENCODE_SYMBOL( context, idx[ pos ] );
			}
		}
	}
	else
	{
		...
		{
			_pep_context* row_first_context = &nw_rows[ idx[ pos - width ] ][ index_escape ];
			_PEP_DIRECT_ENCODE_SYMBOL( row_first_context, idx[ pos ] );
			pos++;
...
#undef _PEP_DIRECT_ENCODE_SYMBOL
#undef _PEP_DIRECT_ARITH_ENCODE
...
// Raw-index optimized mode. The relaxed optimized contract permits a different
// self-contained stream format as long as optimized decompression reproduces
// the same pixels. Model values 0..2 remain the reference arithmetic models;
// model 3 stores one palette index byte per pixel.
#define _PEP_PAL_HASH_SIZE 4096
typedef struct {
    uint32_t color;
    uint8_t index;
    uint8_t occupied;
...
static inline uint32_t _pep_pal_hash(uint32_t x) {
    x ^= x >> 16;
    x *= 0x7feb352du;
    x ^= x >> 15;
    return x & (_PEP_PAL_HASH_SIZE - 1);
}
...
pep pep_compress( const uint32_t* in_pixels, const uint16_t width, const uint16_t height, const pep_format in_format, const pep_channel_bits in_channel_bits )
{
	...
	if( !idx ) return out_pep;
	
	uint32_t palette[ 256 ];
	uint16_t count = 0;
	
	_pep_pal_slot hash_table[_PEP_PAL_HASH_SIZE];
	for (int i = 0; i < _PEP_PAL_HASH_SIZE; i++) {
	...
	{
		uint32_t c = *p;
		uint32_t h = _pep_pal_hash(c);
		uint8_t found_idx = 0;
		uint8_t is_new = 0;
		
		while(1) {
		    if (!hash_table[h].occupied) {
		        is_new = 1;
		        break;
		    }
		    if (hash_table[h].color == c) {
		        found_idx = hash_table[h].index;
		        break;
		    }
		    h = (h + 1) & (_PEP_PAL_HASH_SIZE - 1);
		}
		...
		if (is_new) {
		    if (count < 256) {
		        found_idx = count;
		        palette[count] = c;
		        hash_table[h].color = c;
		        hash_table[h].index = count;
	...
	const uint8_t early_model_order_low[ PEP_NUM_MODELS ] = { 0, 1, 2 };
	const uint8_t early_model_order_high_large[ PEP_NUM_MODELS ] = { 1, 2, 0 };
	const uint8_t early_model_order_high_small[ PEP_NUM_MODELS ] = { 2, 1, 0 };
	const uint8_t* early_model_order = early_model_order_low;
	if( pcount >= 128 )
	{
		early_model_order = area >= 4000000u ? early_model_order_high_large : early_model_order_high_small;
	}

	// Early single-model direct return DISABLED: committing to one heuristic-
	// guessed model made the optimized .pep larger than the reference on real
	// images. Fall through to the strict all-model selection (true minimum).
	if( 0 && area >= 400000u && _pep_models[ early_model_order[ 0 ] ] != _PEP_TAP_HASH )
	{
		const uint8_t model = early_model_order[ 0 ];
		uint8_t* bytes = ( uint8_t* )PEP_MALLOC( ( size_t )area * sizeof( uint32_t ) + 64 );
		if( !bytes )
		{
			PEP_FREE( idx );
			return out_pep;
		...
		const uint64_t size = _pep_encode_model_direct_wnw( idx, width, height, pcount, model, bytes );
		if( size == _PEP_ENCODE_MODEL_ABANDONED )
		{
			PEP_FREE( bytes );
			PEP_FREE( idx );
			return out_pep;
		...
		uint8_t* shrink = ( uint8_t* )PEP_REALLOC( bytes, size );
		if( !shrink && size != 0 )
		{
			PEP_FREE( bytes );
			PEP_FREE( idx );
		...
		out_pep.bytes = shrink;
		out_pep.bytes_size = size;
		out_pep.model = model;
		PEP_FREE( idx );
		return out_pep;
	}

	uint32_t idx_pad_stride = ( uint32_t )width + 3u;
	uint32_t idx_pad_rows = ( uint32_t )height + 2u;
	uint16_t* idx_pad = ( uint16_t* )PEP_MALLOC( ( size_t )idx_pad_stride * idx_pad_rows * sizeof( uint16_t ) );
	if( !idx_pad )
	{
		PEP_FREE( idx );
		return out_pep;
	...
	for( uint32_t index = 0; index < idx_pad_stride * idx_pad_rows; ++index )
	{
		idx_pad[ index ] = ( uint16_t )pcount;
	}
	...
	for( uint32_t y = 0; y < height; ++y )
	{
		...
		{
			idx_pad[ ( y + 2u ) * idx_pad_stride + ( x + 2u ) ] = ( uint16_t )idx[ y * width + x ];
		}
	}
	...
	const uint8_t model_order_low[ PEP_NUM_MODELS ] = { 0, 1, 2 };
	const uint8_t model_order_high_large[ PEP_NUM_MODELS ] = { 1, 2, 0 };
	const uint8_t model_order_high_small[ PEP_NUM_MODELS ] = { 2, 1, 0 };
	const uint8_t* model_order = model_order_low;
	if( pcount >= 128 )
	{
		model_order = area >= 4000000u ? model_order_high_large : model_order_high_small;
	}

	// Strict: always evaluate ALL models and keep the smallest (true minimum,
	// like the reference) so the optimized .pep is never larger than reference.
	const uint8_t model_trials = PEP_NUM_MODELS;
	if( model_trials == 1u )
	{
		const uint8_t model = model_order[ 0 ];
		uint8_t* bytes = ( uint8_t* )PEP_MALLOC( ( size_t )area * sizeof( uint32_t ) + 64 );
		if( !bytes )
		{
			PEP_FREE( idx_pad );
		...
		const uint64_t size = _pep_encode_model( idx, idx_pad, idx_pad_stride, width, height, pcount, model, bytes, 0 );
		if( size == _PEP_ENCODE_MODEL_ABANDONED )
		{
			PEP_FREE( bytes );
			PEP_FREE( idx_pad );
	...
	for( uint8_t model_index = 0; model_index < model_trials; model_index++ )
	{
		const uint8_t model = model_order[ model_index ];
		uint64_t abandon_at = 0;
		if( best_size != 0 )
		{
			abandon_at = model < best_model ? best_size + 1 : best_size;
		}

		uint64_t size = _PEP_ENCODE_MODEL_ABANDONED;
		if( _pep_models[ model ] != _PEP_TAP_HASH )
		{
			// NW / W: fast direct encoder (no count-only/abandon mode, so always
			// full-encode into tmp; it is fast). We still keep the smallest across
			// all models, so the chosen size matches the reference.
			size = _pep_encode_model_direct_wnw( idx, width, height, pcount, model, tmp );
		}
		else if( best_size == 0 )
		{
			size = _pep_encode_model( idx, idx_pad, idx_pad_stride, width, height, pcount, model, tmp, abandon_at );
		}
		else
		{
			size = _pep_encode_model( idx, idx_pad, idx_pad_stride, width, height, pcount, model, NULL, abandon_at );
			if( size == _PEP_ENCODE_MODEL_ABANDONED ) continue;
			if( !( size < best_size || ( size == best_size && model < best_model ) ) ) continue;

			size = _pep_encode_model( idx, idx_pad, idx_pad_stride, width, height, pcount, model, tmp, 0 );
		}
		if( size == _PEP_ENCODE_MODEL_ABANDONED ) continue;
		
		if( best_size == 0 || size < best_size || ( size == best_size && model < best_model ) )
		{
			best_size = size;
			best_model = model;
			uint8_t* swap = best;
	...
	uint8_t* encoded_bytes = ( uint8_t* )PEP_REALLOC( best, best_size );
	if( !encoded_bytes && best_size != 0 )
	{
		PEP_FREE( idx_pad );
		PEP_FREE( idx );
		return out_pep;
...
{
	_pep_decode_model_context m;
	if( !_pep_decode_model_init( &m, _pep_models[ model ], pcount ) ) return;
	const uint32_t index_escape = m.index_escape;
	...
	for( uint32_t y = 0; y < height; y++ )
	{
		for( uint32_t x = 0; x < width; x++, pos++ )
		{
			_pep_decode_context* context = _pep_decode_build_level( &m, idx, pos, x, y, width );
			uint32_t symbol = 0;
			...
			if( context->sum != 0 )
			{
				...
				_pep_sym_decode r = _pep_decode_get_sym_from_freq( context, f );
				_pep_arith_decode_update( &ac, r.prob );

				if( r.symbol != index_escape )
				{
					symbol = r.symbol;
					_pep_decode_context_update( context, symbol, &freq_max, pcount );
					decoded_in_level = 1;
				}
				else
				{
					_pep_decode_context_add_freq( context, index_escape, 1 );
				}
			}
			...
			if( !decoded_in_level )
			{
				...
				_pep_sym_decode r = _pep_decode_get_sym_from_freq( order0, f );
				_pep_arith_decode_update( &ac, r.prob );
				symbol = r.symbol;
				_pep_decode_context_update( order0, symbol, &freq_max, pcount );
				...
				{
					_pep_decode_context_add_freq( context, symbol, 1 );
				}
			}
...
uint32_t* pep_decompress( const pep* const in_pep, const pep_format out_format, const uint8_t transparent_first_color, uint8_t const pre_multiply )
{
	...
	if( in_pep->model == _PEP_MODEL_RAW_INDEX )
	{
		if( in_pep->bytes_size < area )
		{
			PEP_FREE( out_pixels );
			return NULL;
		}
		...
		{
			uint32_t symbol = in_pep->bytes[ pos ];
			uint32_t pixel = ( symbol < pcount ) ? _pep_reformat( palette[ symbol ], in_pep->format, out_format ) : 0;
			if( pre_multiply != 0 ) pixel = _pep_pre_multiply( pixel, out_format );
			out_pixels[ pos ] = pixel;
		}
